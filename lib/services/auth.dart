...
class AuthService {
  ...
  // sign in anon (tutorial)
  Future signInAnon() async {
    try {
      auth.UserCredential result = await _auth.signInAnonymously(); // request authentication result from Firebase
      auth.User user = result.user; //
      return _userFromFirebaseUser(user); // We return custom user object
    } catch (e) {
      print(e.toString()); // replace with snack bar
      return null; //
    }
  }
